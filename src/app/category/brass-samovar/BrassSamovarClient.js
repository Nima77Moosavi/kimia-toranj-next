...
export default function BrassSamovarClient({
  initialCollection,
  initialProducts,
  initialHasMore,
}) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}api/store/products/?collection=سماور برنجی&page=${page}`)
      .then((res) => {
  ...
  return (
    <div className={styles.brassSamovarPage}>
      <div className={styles.header}>
        {/* Header is already in server component */}
      </div>

      <section className={styles.productGridSection}>
        <div className={styles.container}>
          <h1 className={styles.sectionTitle}>
            {initialCollection.title || "سماور برنجی کیمیا ترنج"}
          </h1>
